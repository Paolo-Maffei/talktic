...
#define UNPACK_NEED(n) \
    do { \
  	    if (bufpos + (n) > buflen) { \
	        sprintf(vm->error, "String.%s(): too short string for the format", js_vm_symname (vm, method)); \
	        js_vm_error(vm); \
        } \
    } while (0)
#else
       ...
       JSNode * args)
{
    ...
    /* ********************************************************************** */
        ...
        for (op = 0; op < args[1].u.vstring->len; op++) {
            ...
            case 'N':
                ...
                buffer[bufpos++] =
                    (unsigned char) ((ui & 0xff000000) >> 24);
                buffer[bufpos++] =
                    (unsigned char) ((ui & 0x00ff0000) >> 16);
                buffer[bufpos++] =
                    (unsigned char) ((ui & 0x0000ff00) >> 8);
                buffer[bufpos++] = (unsigned char) ((ui & 0x000000ff));
                break;
    ...
    /* ********************************************************************** */
        ...
        /* ***************************************************************** */
            ...
            for (i = 0; i < args->u.vinteger; i++) {
                memcpy(result_return->u.vstring->data + pos,
                       args[i + 1].u.vstring->data,
                       args[i + 1].u.vstring->len);
        ...
        /* ***************************************************************** */
            ...
            if (start_index >= 0
                && start_index + args[1].u.vstring->len <=
                n->u.vstring->len) {
                /* Use the Brute Force Luke! */
                for (;
                     start_index + args[1].u.vstring->len <=
                     n->u.vstring->len; start_index++)
                    if (memcmp
                        (n->u.vstring->data + start_index,
        ...
        /* ***************************************************************** */
            ...
            js_builtin_RegExp_match(vm, n->u.vstring->data,
        ...
        /* ***************************************************************** */
            ...
            js_builtin_RegExp_replace(vm, n->u.vstring->data,
                                      n->u.vstring->len, &args[1],
                                      args[2].u.vstring->data,
                                      args[2].u.vstring->len,
                                      result_return);
        }
        /* ***************************************************************** */
            ...
            js_builtin_RegExp_search(vm, n->u.vstring->data,
                                     n->u.vstring->len, &args[1],
                                     result_return);
        }
#endif
                ...
                if (args[1].type == JS_STRING) {
                    ...
                    for (pos = 0;
                         (alen < limit
                          ...
                          n->u.vstring->len);) {
                        if (memcmp
                            (n->u.vstring->data + pos,
                             args[1].u.vstring->data,
                             args[1].u.vstring->len) == 0) {
                            /* Found the separator. */
                            js_vm_expand_array(vm, result_return,
                                               alen + 1);
                            js_vm_make_string(vm,
                                              &(result_return->u.varray->
                                                data[alen]),
...
/* New proc. */
static void
new_proc(JSVirtualMachine * vm, JSBuiltinInfo * builtin_info,
...
void js_builtin_String(JSVirtualMachine * vm)
{
    StringCtx *ctx;
    JSNode *n;
    JSBuiltinInfo *info;
