    ...
    if (setjmp(vm->error_handler->error_jmp)) {
        ...
        for (sect = 0; sect < bc->num_sects; sect++)
            if (bc->sects[sect].type == JS_BCST_CONSTANTS) {
                ...
                for (ui = 0; ui < bc->sects[sect].length;) {
                    ...
                    case JS_STRING:
                        c->u.vstring =
                            js_vm_alloc(vm, sizeof(*c->u.vstring));
                        ...
                        c->u.vstring->data =
                            js_malloc(vm, c->u.vstring->len + 1);
                        memcpy(c->u.vstring->data, cp + ui,
                               c->u.vstring->len);
        ...
        for (sect = 0; sect < bc->num_sects; sect++)
            if (bc->sects[sect].type == JS_BCST_SYMTAB) {
                ...
                symtab =
                    js_calloc(vm, num_symtab_entries + 1, sizeof(*symtab));
...
#ifdef _RUNTIME_WARNING
                sprintf(vm->error,
                        "illegal builtin object for call_method");
#endif
                result = 0;
            }
...
static void destroy_builtin(void *ptr)
{
    JSBuiltin *bi = ptr;
...
static void destroy_builtin_info(void *ptr)
{
    JSBuiltinInfo *i = ptr;
