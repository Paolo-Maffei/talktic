...
static inline unsigned int list(unsigned int size)
{
    unsigned int list = 0;
    size >>= 3;
...
static inline void delete_destroyable(JSHeapMemoryBlock * b)
{
...
static unsigned long sweep(JSVirtualMachine * vm)
{
    JSHeapBlock *hb;
    ...
    for (hb = vm->heap; hb; hb = hb->next) {
        ...
        b = (JSHeapMemoryBlock *) ((unsigned char *) hb +
                                   sizeof(JSHeapBlock));
        e = (JSHeapMemoryBlock *) ((unsigned char *) hb +
                                   sizeof(JSHeapBlock)
                                   + hb->size);
        for (; b < e; b = bnext) {
#if JS_MEM_DEBUG
...
void *js_vm_realloc(JSVirtualMachine * vm, void *ptr,
    ...
    case JS_BUILTIN:
        if (js_vm_mark_ptr(n->u.vbuiltin)) {
            ...
            if (n->u.vbuiltin->info->mark_proc)
                (*n->u.vbuiltin->info->mark_proc) (n->u.vbuiltin->info,
                                                   n->u.vbuiltin->
                                                   instance_context);
        }
        break;
