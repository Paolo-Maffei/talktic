                   ...
                   JSNode * result_return, JSNodeType preferred_type)
{
    ...
    case JS_OBJECT:
        if (preferred_type == JS_STRING) {
            ...
            else if (js_vm_call_method
                     (vm, (JSNode *) n, "valueOf", 0, &args)
                     && JS_IS_PRIMITIVE_VALUE(&vm->exec_result))
                JS_COPY(result_return, &vm->exec_result);
            else {
...
void
js_vm_to_string(JSVirtualMachine * vm, const JSNode * n,
                JSNode * result_return)
{
    ...
    case JS_BUILTIN:
        if (n->u.vbuiltin->info->method_proc)
            if ((*n->u.vbuiltin->info->method_proc) (vm,
                                                     n->u.vbuiltin->info,
                                                     n->u.vbuiltin->
                                                     instance_context,
                                                     vm->syms.s_toString,
    ...
    js_vm_make_static_string(vm, result_return, tostring,
                             strlen(tostring));
}


void
js_vm_to_number(JSVirtualMachine * vm, const JSNode * n,
                JSNode * result_return)
{
    char *cp, *end;
...
void
js_vm_to_object(JSVirtualMachine * vm, const JSNode * n,
                JSNode * result_return)
{
    switch (n->type) {
    case JS_BOOLEAN:
