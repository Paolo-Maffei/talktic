...
function JSC$parser_get_semicolon_asci(stream)
{
    ...
    if (token == '}'.charCodeAt(0)
        || JSC$parser_token_linenum < JSC$parser_peek_token_linenum || token == JSC$tEOF) {
...
function JSC$parser_expr_is_left_hand_side(expr)
{
    return (expr.etype == JSC$EXPR_CALL
            ...
            || expr.etype == JSC$EXPR_TRUE || expr.etype == JSC$EXPR_FALSE);
}
...
function JSC$parser_parse_function_declaration(stream)
{
    ...
    JSC$functions.push(new JSC$function_declaration(ln, lbrace_ln, id,
                                                    id_given, args, block, use_arguments));
...
function JSC$parser_parse_block(stream)
{
    ...
    if (JSC$parser_peek_token(stream) != '{'.charCodeAt(0))
        return false;

    JSC$parser_get_token(stream) != '{'.charCodeAt(0);
...
function JSC$parser_parse_stmt(stream)
{
    ...
    if (typeof(item = JSC$parser_parse_block(stream)) != "boolean")
        ...
        if (item.etype == JSC$EXPR_IDENTIFIER) {
            ...
            if (token == ':'.charCodeAt(0) && item.linenum == JSC$parser_peek_token_linenum) {
                /* Yes it is. */
                JSC$parser_get_token(stream);
                var stmt = JSC$parser_parse_stmt(stream);
                if (!stmt)
                ...
                return new JSC$stmt_labeled_stmt(item.linenum, item.value, stmt);
            }
            /* FALLTHROUGH */
        }
...
function JSC$parser_parse_switch(stream)
{
    ...
    while (true) {
        ...
        if (token == '}'.charCodeAt(0))
            ...
            while (true) {
                token = JSC$parser_peek_token(stream);
                if (token == '}'.charCodeAt(0) || token == JSC$tCASE || token == JSC$tDEFAULT)
    ...
    return new JSC$stmt_switch(linenum, JSC$parser_token_linenum, expr, clauses);
}
...
function JSC$parser_parse_try(stream)
{
    ...
    return new JSC$stmt_try(linenum, try_block_last_linenum,
                            JSC$parser_token_linenum, block, catch_list, fin);
}
...
function JSC$parser_parse_relational_expr(stream)
{
    ...
    token = JSC$parser_peek_token(stream);
    while (token == '<'.charCodeAt(0) || token == '>'.charCodeAt(0) || token == JSC$tLE
           || token == JSC$tGE) {
        JSC$parser_get_token(stream);
        var ln = JSC$parser_token_linenum;
...
function JSC$parser_parse_additive_expr(stream)
{
    ...
    token = JSC$parser_peek_token(stream);
    while (token == '+'.charCodeAt(0) || token == '-'.charCodeAt(0)) {
        JSC$parser_get_token(stream);
        var ln = JSC$parser_token_linenum;
...
function JSC$parser_parse_multiplicative_expr(stream)
{
    ...
    token = JSC$parser_peek_token(stream);
    while (token == '*'.charCodeAt(0) || token == '/'.charCodeAt(0) || token == '%'.charCodeAt(0)) {
        JSC$parser_get_token(stream);
        var ln = JSC$parser_token_linenum;
...
function JSC$parser_parse_unary_expr(stream)
{
    ...
    if (token == JSC$tDELETE
        || token == JSC$tVOID
        || token == JSC$tTYPEOF
        || token == JSC$tPLUSPLUS || token == JSC$tMINUSMINUS || token == '+'.charCodeAt(0)
        || token == '-'.charCodeAt(0)
        || token == '~'.charCodeAt(0)
        || token == '!'.charCodeAt(0)) {
        JSC$parser_get_token(stream);
...
function JSC$parser_parse_left_hand_side_expr(stream)
{
    ...
    while ((token = JSC$parser_peek_token(stream)) == '('.charCodeAt(0)
           || token == '['.charCodeAt(0) || token == '.'.charCodeAt(0)) {
        var ln = JSC$parser_peek_token_linenum;
        ...
        if (token == '('.charCodeAt(0)) {
            ...
            expr = new JSC$expr_object_property(ln, expr, JSC$parser_token_value);
        }
    }
...
function JSC$parser_parse_primary_expr(stream)
{
    ...
    if (token == JSC$tTHIS)
        ...
        while ((token = JSC$parser_peek_token(stream)) != ']'.charCodeAt(0)) {
            ...
            token = JSC$parser_peek_token(stream);
            if (token != ','.charCodeAt(0) && token != ']'.charCodeAt(0))
                JSC$parser_syntax_error();
        }
        ...
        while ((token = JSC$parser_peek_token(stream)) != '}'.charCodeAt(0)) {
            ...
            if (token != JSC$tIDENTIFIER && token != JSC$tSTRING && token != JSC$tINTEGER)
                JSC$parser_syntax_error();
            ...
            if (token == ','.charCodeAt(0)) {
                ...
                if (token != JSC$tIDENTIFIER && token != JSC$tSTRING && token != JSC$tINTEGER)
                    JSC$parser_syntax_error();
            } else if (token != '}'.charCodeAt(0) && token)
                JSC$parser_syntax_error();
        }
...
{
    this.top = new JSC$ContBreakFrame(loop_break, loop_continue, inswitch, label, this.top);
}
...
JSC$ContBreak.prototype.count_switch_nesting = JSC$ContBreak$count_switch_nesting;
...
function JSC$function_declaration(ln, lbrace_ln, name, name_given, args, block, use_arguments_prop)
{
    this.linenum = ln;
...
function JSC$stmt_function_declaration(ln, container_id, function_id, given_id)
{
    ...
    if (l_cont == null) {
        if (this.label)
            error(JSC$filename + ":" + this.linenum.toString()
                  + ": label `" + this.label + "' not found for continue statement");
        else
            error(JSC$filename + ":" + this.linenum.toString()
...
function JSC$stmt_break_asm()
{
    ...
    if (l_break == null) {
        if (this.label)
            error(JSC$filename + ":" + this.linenum.toString()
                  + ": label `" + this.label + "' not found for break statement");
        else
            error(JSC$filename + ":" + this.linenum.toString()
...
function JSC$stmt_try(ln, try_block_last_ln, try_last_ln, block, catch_list, fin)
{
...
function JSC$stmt_try_asm()
{
    ...
    if (this.catch_list) {
        ...
        new JSC$ASM_try_push(this.catch_list.linenum, l_catch_list_error).link();
        JSC$cont_break.top.try_nesting++;
        ...
        for (i = 0; i < this.catch_list.length; i++) {
            ...
            JSC$ns.push_frame();
            JSC$ns.define_symbol(c.id, JSC$SCOPE_LOCAL, JSC$ns.alloc_local(), c.linenum);

            new JSC$ASM_dup(c.linenum).link();
            new JSC$ASM_store_local(c.linenum, JSC$ns.lookup_symbol(c.id).value).link();
        ...
        new JSC$ASM_iftrue_b(this.catch_list.last_linenum, l_we_did_enter).link();
...
function JSC$stmt_for_asm()
{
    ...
    if (this.vars) {
        ...
        for (i = 0; i < this.vars.length; i++) {
            ...
            JSC$ns.define_symbol(decl.id, JSC$SCOPE_LOCAL, JSC$ns.alloc_local(), this.linenum);
    ...
    if (this.expr2 != null) {
        this.expr2.asm();
        if (JSC$optimize_type && this.expr2.lang_type && this.expr2.lang_type == JSC$JS_BOOLEAN)
            type_op = true;
    } else {
...
function JSC$stmt_variable_asm()
{
    ...
    for (j = 0; j < this.list.length; j++) {
        ...
        if (!this.global_level)
            JSC$ns.define_symbol(i.id, JSC$SCOPE_LOCAL, JSC$ns.alloc_local(), this.linenum);
        if (i.expr) {
            i.expr.asm();
...
function JSC$expr_additive_constant_folding()
{
    ...
    if (this.e1.lang_type && this.e2.lang_type && this.e1.lang_type == this.e2.lang_type) {
        switch (this.e1.lang_type) {
        case JSC$JS_INTEGER:
            return new JSC$expr_integer(this.linenum, this.type == '+'.charCodeAt(0)
                                        ? this.e1.value + this.e2.value
                                        : this.e1.value - this.e2.value);
            break;

        case JSC$JS_FLOAT:
            return new JSC$expr_float(this.linenum, this.type == '+'.charCodeAt(0)
                                      ? this.e1.value + this.e2.value
                                      : this.e1.value - this.e2.value);
            break;
        ...
        case JSC$JS_STRING:
            if (this.type == '+'.charCodeAt(0))
                ...
                return new JSC$expr_string(this.linenum, this.e1.value + this.e2.value);
            break;
...
function JSC$expr_new_asm()
{
    ...
    new JSC$ASM_apop(this.linenum, (this.args ? this.args.length : 0) + 2).link();
}
...
if (this.type == JSC$tDIVA)
    new JSC$ASM_div(this.linenum).link();
    else
if (this.type == JSC$tMODA)
    new JSC$ASM_mod(this.linenum).link();
    else
if (this.type == JSC$tADDA)
    new JSC$ASM_add(this.linenum).link();
    else
if (this.type == JSC$tSUBA)
    new JSC$ASM_sub(this.linenum).link();
    else
if (this.type == JSC$tLSIA)
    new JSC$ASM_shift_left(this.linenum).link();
    else
if (this.type == JSC$tRSIA)
    new JSC$ASM_shift_right(this.linenum).link();
    else
if (this.type == JSC$tRRSA)
    new JSC$ASM_shift_rright(this.linenum).link();
    else
if (this.type == JSC$tANDA)
    new JSC$ASM_and(this.linenum).link();
    else
if (this.type == JSC$tXORA)
    new JSC$ASM_xor(this.linenum).link();
    else
if (this.type == JSC$tORA)
    new JSC$ASM_or(this.linenum).link();
    else
    error(JSC$filename + ":" + this.linenum.toString()
...
function JSC$expr_unary_asm()
{
    if (this.type == '!'.charCodeAt(0)) {
    ...
    } else if (this.type == JSC$tPLUSPLUS || this.type == JSC$tMINUSMINUS) {
        /* Fetch the old value. */
        JSC$asm_expr_lvalue_load_asm(this.expr);
        ...
        error("jsc: internal error: unary expr's type is " + this.type.toString());
    }
}
    ...
    for (i = this.frame.defs; i != null; i = i.next)
        if (i.symbol == symbol) {
            ...
            if (i.scope == JSC$SCOPE_ARG && JSC$warn_shadow)
                JSC$warning(JSC$filename + ":" + linenum.toString()
                            + ": warning: declaration of `" + symbol + "' shadows a parameter");
...
function JSC$ASM_const_print(stream)
{
    ...
    else if (typeof this.value == "string" || typeof this.value == "#builtin") {
        var i, c;
        var ender, src;
...
function JSC$ASM_const_bytecode()
{
    ...
    if (typeof this.value == "number") {
        if (isInt(this.value))
            cid = JSC$asm_genconstant(String.pack("CN", JSC$CONST_INT, this.value));
        else if (isFloat(this.value))
            cid = JSC$asm_genconstant(String.pack("Cd", JSC$CONST_FLOAT, this.value));
        else
            cid = JSC$asm_genconstant(String.pack("C", JSC$CONST_NAN));
    } else if (typeof this.value == "string")
        cid = JSC$asm_genconstant(String.pack("CN", JSC$CONST_STRING, this.value.length)
                                  + this.value);
...
function JSC$asm_is_load_op(op)
{
    return (op.type == JSC$OP_LOAD_GLOBAL
            || op.type == JSC$OP_LOAD_ARG || op.type == JSC$OP_LOAD_LOCAL);
}
...
function JSC$asm_is_store_op(op)
{
    return (op.type == JSC$OP_STORE_GLOBAL
            || op.type == JSC$OP_STORE_ARG || op.type == JSC$OP_STORE_LOCAL);
}
...
function JSC$asm_is_local_jump(op)
{
    return (op.type == JSC$OP_JMP
            ...
            || op.type == JSC$OP_IFTRUE_B || op.type == JSC$OP_TRY_PUSH);
}
...
{
    while (item != null && (item.type == JSC$ASM_LABEL || item.type == JSC$ASM_SYMBOL))
        item = item.next;
    ...
    if ((flags & JSC$FLAG_OPTIMIZE_PEEPHOLE) != 0) {
        ...
        for (item = JSC$asm_head; item != null; item = item.next) {
            ...
            if (item.next != null && item.next.type == JSC$OP_DUP) {
                ...
                for (i1 = item.next.next; i1 != null && i1.next != null; i1 = i1.next) {
                    var i2 = i1.next;
                    ...
                    if (JSC$asm_is_local_jump(i1)
                        ...
                        || i1.type == JSC$OP_DUP
                        || i1.type == JSC$OP_TRY_POP || i1.type == JSC$OP_THROW)
                        break;
            ...
            if (item.next != null && item.next.next != null) {
                ...
                if (i1.type == JSC$OP_APOP && i2.type == JSC$OP_POP) {
                    /*
                     * i1:    apop n
                     * i2:    pop     ->  pop_n n + 1
                     */
            ...
            if (item.next != null && item.next.next != null) {
                ...
                if (i1.type == JSC$OP_CONST_TRUE
                    && (i2.type == JSC$OP_IFFALSE || i2.type == JSC$OP_IFFALSE_B)) {
                    /*
                     * i1:    const_true
                     * i2:    iffalse{,_b}    .LX => ---
                     */
            ...
            if (item.next != null && item.next.next != null) {
                ...
                if (i1.type == JSC$OP_CONST_FALSE
                    && (i2.type == JSC$OP_IFTRUE || i2.type == JSC$OP_IFTRUE_B)) {
                    /*
                     * i1:    const_false
                     * i2:    iftrue{,_b} .LX => ---
                     */
        ...
        while (delta) {
            ...
            for (item = JSC$asm_head; item != null; item = item.next)
                while (item.next != null && item.next.type == JSC$ASM_LABEL
                       && !item.next.referenced && item.next.next != null) {
                    delta = true;
                    item.next = item.next.next;
                }
            ...
            for (item = JSC$asm_head; item != null; item = item.next) {
                ...
                if (item.next != null && item.next.next != null) {
                    ...
                    if (i1.type == JSC$OP_JMP && i2.type == JSC$ASM_LABEL && i1.value == i2) {
                        /*
                         * i1:    jmp .LX
                         * i2:    .LX     => .LX
                         */
...
function JSC$optimize_heavy()
{
    ...
    for (ftail = JSC$asm_tail; ftail != null; ftail = fhead.prev) {
        ...
        while (change) {
            ...
            for (fhead = ftail; fhead.type != JSC$ASM_SYMBOL; fhead = fhead.prev) {
                var floc, farg;
                ...
                if (fhead.live_used && (fhead.live_locals != floc || fhead.live_args != farg))
                    change = true;
    ...
    for (item = JSC$asm_head; item != null; item = item.next) {
        ...
        if (item.next != null && item.next.next != null && item.next.next.next != null) {
            var i1 = item.next;
            var i2 = i1.next;
            var i3 = i2.next;
            ...
            if (i1.type == JSC$OP_STORE_LOCAL
                ...
                && i1.value == i2.value && (i3.live_locals & (1 << i1.value)) == 0) {
                /*
                 * i1:    store_local     n
                 * i2:    load_local  n
...
function JSC$asm_genconstant(val)
{
    ...
    JSC$asm_constants = JSC$asm_constants.append(val);
...
function JSC$asm_bytecode()
{
    ...
    if (JSC$generate_debug_info)
        /* Source file name. */
        debug = debug.append(String.pack("CN", JSC$DEBUG_FILENAME, JSC$filename.length)
                             + JSC$filename);
    ...
    for (item = JSC$asm_head; item != null; item = item.next) {
        if (item.type == JSC$ASM_SYMBOL) {
            symtab = symtab.append(item.value + String.pack("CN", 0, item.offset));
            nsymtab_entries++;
            ...
            if (JSC$generate_debug_info)
                if (item.linenum != debug_last_linenum) {
                    debug = debug.append(String.pack("CNN", JSC$DEBUG_LINENUMBER,
                                                     item.offset + item.size, item.linenum));
                    debug_last_linenum = item.linenum;
                }

            if (item.size == 1)
                /* We handle these. */
                code = code.append(String.pack("C", item.type));
            else {
                /*
                 * All operands which take an argument, have a method to create
                 * the byte code entry for their argument.
                 */
                code = code.append(String.pack("C", item.type) + item.bytecode());
            }
        }
    }
    ...
    if (JSC$verbose) {
        ...
        msg += (", headers=" + (32 + (JSC$generate_debug_info ? 8 : 0)).toString()
                + ", total="
                + (code.length + JSC$asm_constants.length + symtab.length
                   + debug.length + 32 + (JSC$generate_debug_info ? 8 : 0)).toString());
        JSC$message(msg);
    }

    return (String.pack("NN", JSC$BC_MAGIC, 3 + (JSC$generate_debug_info ? 1 : 0))
            + String.pack("NN", JSC$BC_SECT_CODE, code.length) + code
            + String.pack("NN", JSC$BC_SECT_CONSTANTS, JSC$asm_constants.length)
            + JSC$asm_constants
...
function JSC$compile_stream(stream, flags, asm_file, bc_file)
{
    ...
    if (stream.open()) {
        try {
            ...
            JSC$warn_unused_argument = ((flags & JSC$FLAG_WARN_UNUSED_ARGUMENT) != 0);
            JSC$warn_unused_variable = ((flags & JSC$FLAG_WARN_UNUSED_VARIABLE) != 0);
            JSC$warn_shadow = ((flags & JSC$FLAG_WARN_SHADOW) != 0);
            JSC$warn_with_clobber = ((flags & JSC$FLAG_WARN_WITH_CLOBBER) != 0);
            JSC$warn_missing_semicolon = ((flags & JSC$FLAG_WARN_MISSING_SEMICOLON) != 0);
            JSC$warn_strict_ecma = ((flags & JSC$FLAG_WARN_STRICT_ECMA) != 0);
            JSC$warn_deprecated = ((flags & JSC$FLAG_WARN_DEPRECATED) != 0);
        ...
        error("jsc: couldn't open input stream \"" + stream.name + "\": " + stream.error);
