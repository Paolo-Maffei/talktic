...
/*
 * $Source: /usr/local/cvsroot/ngs/js/jsc/asm.js,v $
 * $Id: asm.js,v 1.15 1998/10/26 15:25:21 mtr Exp $
 */
...
function JSC$ASM_const_bytecode()
{
    ...
    if (typeof this.value == "number") {
        ...
        cid =
            JSC$asm_genconstant(String.
                                pack("CCN", JSC$CONST_REGEXP, flags, this.value.source.length)
                                + this.value.source);
    } else
...
{
    return (op.type == JSC$OP_LOAD_GLOBAL || op.type == JSC$OP_LOAD_ARG
...
{
    return (op.type == JSC$OP_STORE_GLOBAL || op.type == JSC$OP_STORE_ARG
...
{
    return (op.type == JSC$OP_JMP || op.type == JSC$OP_IFFALSE || op.type == JSC$OP_IFTRUE
            || op.type == JSC$OP_IFFALSE_B || op.type == JSC$OP_IFTRUE_B
...
function JSC$asm_optimize(flags)
{
            ...
            if (item.next != null && item.next.type == JSC$OP_DUP) {
                ...
                for (i1 = item.next.next; i1 != null && i1.next != null; i1 = i1.next) {
                    ...
                    if (JSC$asm_is_local_jump(i1)
                        || i1.type == JSC$OP_JSR || i1.type == JSC$OP_NEW
                        || i1.type == JSC$OP_CALL_METHOD || i1.type == JSC$OP_RETURN
                        || i1.type == JSC$ASM_SYMBOL || i1.type == JSC$ASM_LABEL
                        || i1.type == JSC$OP_DUP || i1.type == JSC$OP_TRY_POP
    ...
    if ((flags & JSC$FLAG_OPTIMIZE_BC_SIZE) != 0) {
        ...
        while (delta) {
            ...
            for (item = JSC$asm_head; item != null; item = item.next)
                while (item.next != null && item.next.type == JSC$ASM_LABEL && !item.next.referenced
                       && item.next.next != null) {
                    delta = true;
                    item.next = item.next.next;
                }
...
function JSC$optimize_heavy()
{
    ...
    for (item = JSC$asm_head; item != null; item = item.next) {
        ...
        if (item.next != null && item.next.next != null && item.next.next.next != null) {
            ...
            if (i1.type == JSC$OP_STORE_LOCAL && i2.type == JSC$OP_LOAD_LOCAL
...
function JSC$asm_bytecode()
{
        ...
        if (item.type == JSC$ASM_SYMBOL) {
            ...
            if (JSC$generate_debug_info)
                if (item.linenum != debug_last_linenum) {
                    debug =
                        debug.append(String.
                                     pack("CNN", JSC$DEBUG_LINENUMBER, item.offset + item.size,
                                          item.linenum));
    ...
    return (String.pack("NN", JSC$BC_MAGIC, 3 + (JSC$generate_debug_info ? 1 : 0))
            + String.pack("NN", JSC$BC_SECT_CODE, code.length) + code + String.pack("NN",
                                                                                    JSC$BC_SECT_CONSTANTS,
                                                                                    JSC$asm_constants.
                                                                                    length)
            + JSC$asm_constants + String.pack("NN", JSC$BC_SECT_SYMTAB,
                                              symtab.length) + symtab +
            (JSC$generate_debug_info ? String.pack("NN", JSC$BC_SECT_DEBUG, debug.length) +
             debug : ""));
}
