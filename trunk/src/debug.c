        ...
        for (n = sp + 1; n != fp - 3; n++) {
            ...
            case JS_STRING:
                if (n->u.vstring->len > STRING_MAX_PRINT_LEN)
                    sprintf(buf, " \"%.*s...\"", STRING_MAX_PRINT_LEN, n->u.vstring->data);
                else
                    sprintf(buf, " \"%.*s\"", (int) n->u.vstring->len, n->u.vstring->data);
                break;
            ...
            case JS_ARGS_FIX:
                sprintf(buf, " <num=%d, delta=%d>", n->u.args_fix.argc, n->u.args_fix.delta);
                break;
