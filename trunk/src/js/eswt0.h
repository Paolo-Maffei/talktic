...
if (JS_SP1->type == JS_BUILTIN) {
    ...
    if (JS_SP1->u.vbuiltin->info->property_proc) {
        if ((*JS_SP1->u.vbuiltin->info->property_proc) (vm,
                                                        JS_SP1->u.
                                                        vbuiltin->info,
                                                        JS_SP1->u.
                                                        vbuiltin->
    ...
    JS_SAVE_REGS();
    if ((*vm->prim[JS_SP1->type]->property_proc) (vm,
                                                  vm->prim[JS_SP1->type],
                                                  JS_SP1, j, 0,
                                                  &builtin_result)
        == JS_PROPERTY_UNKNOWN) {
...
if (JS_SP2->type == JS_BUILTIN) {
    if (JS_SP1->type == JS_INTEGER) {
        ...
        if (JS_SP2->u.vbuiltin->info->property_proc) {
            if ((*JS_SP2->u.vbuiltin->info->property_proc) (vm,
                                                            JS_SP2->u.
                                                            vbuiltin->info,
                                                            JS_SP2->u.
                                                            vbuiltin->
...
if (JS_SP2->type == JS_STRING) {
    ...
    i = js_vm_object_nth(vm, JS_SP2->u.vobject, JS_SP1->u.vinteger,
                         JS_SP2);
    JS_SP1->type = JS_BOOLEAN;
    JS_SP1->u.vboolean = i;
} else {
...
if (JS_SP2->type == JS_INTEGER && JS_SP1->type == JS_INTEGER) {
    ...
    if (l->type == JS_NAN || r->type == JS_NAN)
        ...
        if (r->type == JS_INTEGER) {
            ...
            if (l->u.vinteger == 0 && (JS_IS_POSITIVE_INFINITY(r)
...
if (JS_SP1->type == JS_BUILTIN) {
    ...
    if (JS_SP1->u.vbuiltin->info->method_proc) {
        if ((*JS_SP1->u.vbuiltin->info->method_proc) (vm,
                                                      JS_SP1->u.vbuiltin->
                                                      info,
                                                      JS_SP1->u.vbuiltin->
                                                      instance_context, j,
  ...
  /* Call the global method. */
{
    ...
    if (function->type == JS_BUILTIN
        ...
        JS_SAVE_REGS();
        (*function->u.vbuiltin->info->global_method_proc) (vm,
                                                           function->u.
                                                           vbuiltin->info,
                                                           function->u.
                                                           vbuiltin->
...
if (JS_SP1->type == JS_BUILTIN) {
    ...
    /* WITHCHAIN */
    ERROR
        ("delete_property: not implemented yet for the with-chain objects");
}
  /* The primitive language types. */
  /*
...
if (JS_WITHPTR->u.iptr == NULL) {
    ...
    *ip = 1;
    ...
    ip = js_vm_realloc(vm, ip, sizeof(JSUIntAlign)
                       + ((ui + 1) * sizeof(JSNode)));
    (*ip)++;
    np = (JSNode *) ((unsigned char *) ip + sizeof(JSUIntAlign));
        ...
        for (i = 0; i < ui; i++) {
            ...
            if (w->type == JS_BUILTIN) {
                ...
                if (w->u.vbuiltin->info->property_proc)
                    result = (*w->u.vbuiltin->info->property_proc) (vm,
                                                                    w->u.
                                                                    vbuiltin->
                                                                    info,
                                                                    w->u.
        ...
        for (i = 0; i < ui; i++) {
            ...
            if (w->type == JS_BUILTIN) {
                ...
                if (w->u.vbuiltin->info->method_proc)
                    result = (*w->u.vbuiltin->info->method_proc) (vm,
                                                                  w->u.
                                                                  vbuiltin->
                                                                  info,
                                                                  w->u.
